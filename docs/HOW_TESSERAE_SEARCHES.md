...
Each channel produces its own list of candidate parallels with its own scores. The fusion step combines these nine lists into a single ranked result.

The system uses **weighted score fusion**: each channel's score is multiplied by a weight reflecting how precise that channel has proven to be, and the weighted scores are added together. Channels that produce fewer but more reliable results receive higher weights. Edit distance and sound similarity, for example, are weighted most heavily because when they detect a match, it is very likely to be a genuine parallel. The single-word lemma channel, which casts a wider net, receives a lower weight.

On top of the weighted sum, the system adds a **convergence bonus** that rewards pairs found independently by multiple channels. If six out of nine channels all flag the same pair of lines, that agreement is strong evidence of a real connection — stronger than any single channel's score alone. The convergence bonus increases with each additional channel that confirms the finding.

The result is a single ranked list in which the most likely genuine parallels rise to the top. A pair found by six channels with high scores will outrank a pair found by one channel with a moderate score, which is exactly the behavior a scholar would want.
...
The combination of IDF weighting and distance scoring means that the system does not need a traditional stopword list — the kind of manually curated list of common words that older systems required. High-frequency words like *et*, *in*, and *non* automatically receive near-zero IDF scores, so they contribute almost nothing even when they match. The multi-channel fusion further deprioritizes function-word coincidences, since most channels are effectively blind to common vocabulary.
