...
Fusion search is live on the dev website. Production fusion: **91.0% recall** across 5 benchmarks (784/862). Lucan-Vergil: 92.0% (196/213), Achilleid-Thebaid: 96.2% (50/52) — both surpass Phase 2. Key improvements: restored syntax channel (DB-backed), dictionary on windows, channel optimizations (dictionary inverted-index 850×, lemma_min1 IDF pre-filter 20×, syntax caching+multiprocessing). **Pair-size gates removed** — all 9 channels run unconditionally on every text pair. Aeneid × Metamorphoses completes in ~12 min with all channels. Progressive SSE streaming with "Pause updates" button. **Config E weights** (grid-search optimized Feb 27): R@500 improved 19.8%→22.6%. **Stopword penalty** (0.2×) demotes all-function-word pairs. Weight optimization study: `research/studies/2026-02-27_weight_optimization/REPORT.md`. Full evaluation: `research/studies/fusion_experiment_phase2/`. To-do list: `research/sessions/TODO_2026-02-23.md`.
