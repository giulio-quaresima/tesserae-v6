...
Fusion search is live on the dev website. Production fusion: **90.7% recall** across 5 benchmarks (782/862). Lucan-Vergil: 91.5% (195/213), Achilleid-Thebaid: 96.2% (50/52) — both surpass Phase 2. Key improvements: restored syntax channel (DB-backed), dictionary on windows, channel optimizations (dictionary inverted-index 850x, lemma_min1 IDF pre-filter 20x, syntax caching+multiprocessing). **Pair-size gates removed** -- all 9 channels run unconditionally on every text pair. Aeneid x Metamorphoses completes in ~12 min with all channels. Progressive SSE streaming with "Pause updates" button. **Config K -- Three-layer rarity scoring + single-word penalty** (current): Uses geometric mean corpus-IDF (1,429 Latin texts) as the basis for three distinct rarity effects: (1) **squared base multiplier** -- common-word pairs have base scores reduced by up to 96%; (2) **squared IDF-weighted convergence** -- convergence bonus weighted by `min(1.0, geom_idf)^2`, eliminating convergence credit for common-word pairs; (3) **convergence-scaled rarity boost** -- pairs sharing rare words across multiple detection methods get an additive boost scaled by `min(channel_factor, word_factor)` where `channel_factor=(n_channels-1)/5` and `word_factor=(n_unique_words-1)/3` (weight=0.5, cap=2.0). **Single-word penalty** (commit `2d9d520`): `SINGLE_WORD_PENALTY=0.25` applied to rarity multiplier when `n_unique_words <= 1`, before Layer 1 squaring; convergence bonus zeroed for single-word matches. On Aen.7 vs Punica 2, first unigram drops from rank 49 to ~761; zero unigrams in top 500. **Min-IDF gate** (commit `8f38ec7`): `RARITY_MIN_IDF_THRESHOLD=0.5`, `RARITY_MIN_IDF_PENALTY=0.5` -- if ANY word in a pair has corpus IDF < 0.5, multiplier halved before squaring. **Headword IDF normalization** (commit `8f38ec7`): `_get_corpus_doc_freqs()` uses `max(lemma_df, headword_df)` via `latin_lemmas.json` to fix oblique forms (quem→qui, quos→qui). **No-significant-words penalty**: `NO_SIGNIFICANT_WORDS_PENALTY=0.5` -- bigrams where NO word has IDF >= 1.5 (the rarity threshold) get a 0.5 multiplier penalty. These are common-vocabulary pairs (num+campus, ter+centum, mars+saeuus) that carry weak allusion signal. Convergence preserved (zeroing was too aggressive for recall). On Aen.7 vs Punica 2: mars+saeuus drops from rank 13 to 17, num+campus from rank 23 to 29, ter+centum from 16 to 19. Recall unchanged at 782/862. **Many-to-one fix** (commit `b8b3adb`): `n_unique_words = min(src_words, tgt_words)` prevents synonym mapping from inflating word counts; scorer skips phantom dictionary entries. **Sub-lexical penalty** (commit `c7ae13a`): matches with no lexical evidence get `multiplier=idf_floor` (0.2) instead of free pass (1.0); eliminated sub-lexical-only junk from top results. Earlier fixes (commit `6732e0a`): (A) idf=0 lexical entries included in rarity; (B) surface form deduplication; (C) word-count factor for Layer 3 boost. IDF floor=0.2, threshold=1.5. Channel weights: ed=2.0, sn=4.0, ex=1.0, lm=2.0, rw=2.0, dc=0.5, se=1.2, sy=0.3, m1=0.3. Convergence bonus=0.75. Total recall: 782/862 (90.7%). Study report: `research/studies/2026-02-27_weight_optimization/REPORT.md`. To-do list: `research/sessions/TODO_2026-02-23.md`. **Next planned work:** DHQ article writing tool (plan at `research/plans/article_writing_plan.md`, waiting for user to upload updated V6 dev draft).
